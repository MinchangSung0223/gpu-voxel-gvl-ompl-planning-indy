    ...
    , ob::MotionValidator(si)
{
    ...
    gvl->addRobot("myUrdfRobot", "indy7_ros.urdf", true);
...
signal(SIGINT, ctrlchandler);
  ...
  const Vector3f camera_offsets(0.75f,1.45f,1.5f);  // camera located at y=-0.2m, x_max/2, z_max/2
  ...
  tf = Matrix4f::createFromRotationAndTranslation(Matrix3f::createFromRPY(-PI/2+ roll, 0+pitch, -PI/2+  yaw), camera_offsets);
